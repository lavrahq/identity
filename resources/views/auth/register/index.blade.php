...
<div class="flex bg-gray-300 rounded shadow-2xl w-5/6">
    <div class="w-full bg-gray-200 flex flex-col rounded px-6 py-6 shadow-lg">
        ...
        <p class="text-gray-600 text-center font-light w-full mb-2">
            Hey there! Welcome to {{ config('identity.org.name') }}.
        </p>

        <form action="{{ route('auth.register.index') }}" method="POST" class="flex flex-col">
            @csrf

            <input type="hidden" name="email_id" value="{{ $emailId }}" />

            <p class="text-gray-600 text-left text-xs w-full mt-2 mb-2">
